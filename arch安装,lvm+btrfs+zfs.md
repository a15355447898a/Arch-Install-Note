...
本方案的思路是想利用lvm的动态调整分区能力和快照功能;btrfs的透明压缩,快照和子卷功能(最重要的是,这个文件系统在linux内核里面);zfs的快照,压缩和去重功能

**如果你并不需要以上的任何一个功能,请不要使用本方案**

**千万不要看本方案,但是只使用一部分,干出lvm-btrfs这种莫名其妙而且没什么卵用又影响性能的方案来!!!!**

启动方式是bios+efi是为了考虑有的硬盘不支持其中一个启动方案,这个系统是为了linux to go而准备的(你之后可能会注意到同时安装了`intel`和`amd`的cpu微码)
